...
import { fetchReportTableColumnDefs } from "./report-table-column-def";

interface ReportTableProps {
    reportPromise: ReturnType<typeof getBadReports>;
}

export function ReportsTable({ reportPromise }: ReportTableProps) {

  const featureFlags = useFeatureFlagsStore((state) => state.featureFlags);

  const enableFloatingBar = featureFlags.includes("floatingBar");

  const { data, pageCount } = React.use(reportPromise);

  const columns = React.useMemo<ColumnDef<IBadReport, unknown>[]>(
    () => fetchReportTableColumnDefs(),
    []
  );
