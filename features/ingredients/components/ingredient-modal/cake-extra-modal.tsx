...
const CakeExtraModal = () => {
  ...
  const form = useForm<z.infer<typeof cakeExtraSchema>>({
    ...
    defaultValues: {
      ...
      color: data?.cakeExtra?.color || "",
      is_default: data?.cakeExtra?.is_default || false,
      description: data?.cakeExtra?.description || "",
      image_id: data?.cakeExtra?.image_id || "",
  ...
  useEffect(() => {
    if (data?.cakeExtra?.image && data.cakeExtra.image.file_url) {
      setUploadedFileUrl(data.cakeExtra.image.file_url);
    } else if (data?.cakeExtra?.image_id && !data?.cakeExtra?.image) {
  ...
  }, [isOpenModal, data?.cakeExtra]);

  // Fetch image function
  const fetchImage = async () => {
    if (!data?.cakeExtra?.image_id) return;

    try {
      setFetchingImage(true);
      const result = await getCakeImageById(data.cakeExtra.image_id);
                      ...
                      <div className="relative w-full h-32 border rounded-md border-dashed flex items-center justify-center bg-gray-50/50 group">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <Button
                            ...
                            onClick={() =>
                              document
                                .getElementById("extra-image-upload")
                                ?.click()
                            }
